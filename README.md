...
| `ALLOWED_USER_IDS` | нет | Белый список Telegram ID через запятую; если задан — скачивать могут только они (и админ) |
| `BOT_DEBUG` | нет | Установи `1` для debug-лога |
| `YT_DLP_BIN` | нет | Кастомный путь к yt-dlp |
| `YT_DLP_USER_AGENT` | нет | Кастомный User-Agent для yt-dlp |
