...
// VideoInfo хранит информацию о скачанном видео
type VideoInfo struct {
	filename     string        // Имя файла
	size         int64         // Размер в байтах
	duration     time.Duration // Длительность
	downloadTime time.Duration // Время на скачивание
}
...
func handleVideoURL(bot *telegram.BotAPI, chatID int64, url string) {
	...
	info, _ := os.Stat(filePath)                              // Получаем информацию о файле (размер, дата и т.д.)
	if info != nil && info.Size() > maxFileSizeMB*1024*1024 { // 50 МБ в байтах
...
// Например: 1024 -> "1 KB", 1024*1024 -> "1 MB"
...
// ffprobe входит в состав ffmpeg и позволяет получить метаданные видео
func getVideoDuration(filePath string) time.Duration {
	// Запускаем ffprobe для получения длительности видео
	// -v error — выводить только ошибки
	// -show_entries format=duration — показать только длительность
	// -of default=noprint_wrappers=1:nokey=1 — вывести только число
	...
	// Вывод ffprobe — строка с числом (например, "125.5")
	// Парсим её в float64
	var seconds float64
	...
	// Возвращаем длительность в виде Duration
